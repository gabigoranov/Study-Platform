...
export default function FlashcardDashboardComponent({ flashcard, isSelected, onSelect } : FlashcardDashboardProps) {
    return (
        <div onClick={() => onSelect(flashcard.id)} className={`${isSelected ? "bg-neutral-300 dark:bg-neutral-400" : "bg-background-muted dark:bg-surface"} flex gap-4 justify-between items-center w-auto min-w-[400px] min-h-[200px] rounded-2xl overflow-hidden border border-gray-300 dark:border-gray-800`}>
